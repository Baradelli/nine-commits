...
DECISIONS #66 above contains a number that was already false when it was written, and it stays there as written. It says `search_files('model_name')` returns 5 matches with 3 from the post's own files; the live values at that commit were 7 and 6. The commit that recorded the observation falsified it - it added `runs.tsv` to the project and added a line containing the pattern to the page, and one of the new matches was the disclosure sentence itself, which quotes the string it is describing. Third time this class has bitten in post 2, and the first time it bit a sentence rather than a run.

The post's numbers are FIXED BY DELETION, not by refreshing them. Every live-count assertion is gone: the reproduction section now describes the mechanism, names the three places in the page the matches come from, and tells the reader to run it and read their own number. The "75 paths at the tag" clause became "smaller than it is now, because this page is in it". A figure whose only job is to say the corpus grew is a figure that will be wrong by the next commit, and re-measuring it would only have reset the clock. Every number that survives in the post traces to a committed artifact - the two traces or `runs.tsv` - and is asserted by a 20-check script run before committing. Cost if wrong: the reader gets a mechanism and a command instead of a number, which is the only version that stays true.
