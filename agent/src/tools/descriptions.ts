 ...
 * no globbing, nothing outside the directory, and no way to put content of its
 * own into a file. A description that promised a shell would be measuring how
 ...
 * and it has now been wrong twice.** It read "It cannot change what is inside a
 * file", which `cp a b` falsifies in two tokens. The first correction replaced
 * it with "It can copy, rename and create empty files, but it cannot write new
 * content into one", which is a narrower falsehood: `uniq` takes an optional
 * output operand, so `uniq -c in.txt out.txt` writes `out.txt`, and the count
 * column it writes there was on no disk. The lesson the second round cost is
 * that an enumeration is a claim about everything it leaves out, so this string
 * now states the property — nothing here puts text *you* wrote into a file —
 * and leaves the membership to `tools/shell/writers.test.ts`, which measures it.
 *
 * The correction is worth the same note v7 got: post 8's published context
 * figures (432.9 and 586.9 tokens, a standing charge of 154) are recomputed by
 * `tools/shell/tally.test.ts` from `runs.tsv`, which records what those runs
 * were handed, so they are unchanged — but a reader who checks out this tag and
 * re-runs the sweep is handing the model a slightly longer string than the one
 * that was measured.
 */
...
export const DESCRIPTIONS: Record<DescriptionStyle, ToolDescriptions> = {
  precise: {
    ...
    run_command:
      ...
      'It runs one command, not a shell: no pipes, no redirection, no && or ;, no wildcards, no variables, and nothing outside this directory. Several of these commands write — cp and mv replace a file, mkdir and touch create one, and uniq writes to a second filename if you give it one — but none of them can put text you wrote into a file.',
  },
  thin: {
